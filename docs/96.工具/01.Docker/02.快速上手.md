...
无法访问或者龟速github的可使用以下脚本替换上述脚本

```shell
wget https://dev88.cn/data/docker-compose-linux-x86_64 -O /usr/local/bin/docker-compose && sudo chmod +x /usr/local/bin/docker-compose && sudo ln -s /usr/local/bin/docker-compose /usr/bin/docker-compose && docker-compose --version
```
